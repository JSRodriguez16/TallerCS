...
public class Main {
    ...
    private static List<Empleado> generarEmpleadosAleatorios(int cantidad, Random random) {
        ...
        for (int i = 0; i < cantidad; i++) {
            ...
            float tarifaHoras = 5000 + random.nextInt(10000);
            int sexo = random.nextInt(2);
            int horasTrabajadas = 160 + random.nextInt(181);

            empleados.add(new Empleado(codigo, nombre, tarifaHoras, sexo, horasTrabajadas));
        }
        return empleados;
    }
    ...
    private static void verTodosEmpleados(List<Empleado> empleados) {
        ...
        for (Empleado empleado : empleados) {
            ...
            mensaje.append("Sexo: ").append(empleado.getSexo() == 0 ? "Hombre" : "Mujer").append("\n");
            mensaje.append("Salario Bruto: $").append(String.format("%.2f", empleado.calcularSalarioBruto()))
                    .append("\n");
            mensaje.append("Retencion: $").append(String.format("%.2f", empleado.calcularRetencion())).append("\n");
    ...
    private static void agregarEmpleado(List<Empleado> empleados, Entrada entrada, Salida salida) {
        ...
        String tarifaStr = entrada.recibirInformacion("Ingrese la tarifa por hora:");
        if (tarifaStr == null || tarifaStr.trim().isEmpty()) {
            salida.mostrarMensaje("Tarifa invalida", "Error");
            return;
        }

        float tarifa;
        try {
            tarifa = Float.parseFloat(tarifaStr);
            if (tarifa <= 0) {
                salida.mostrarMensaje("La tarifa debe ser mayor a 0", "Error");
                return;
            }
        } catch (NumberFormatException e) {
            salida.mostrarMensaje("La tarifa debe ser un numero", "Error");
            return;
        }

        String sexoStr = entrada.recibirInformacion("Ingrese el sexo (0=Hombre, 1=Mujer):");
        if (sexoStr == null || sexoStr.trim().isEmpty()) {
            salida.mostrarMensaje("Sexo invalido", "Error");
            return;
        }
        ...
        try {
            ...
            if (sexo != 0 && sexo != 1) {
                salida.mostrarMensaje("El sexo debe ser 0 (Hombre) o 1 (Mujer)", "Error");
                return;
            }
        } catch (NumberFormatException e) {
            salida.mostrarMensaje("El sexo debe ser un numero entero", "Error");
            return;
        }
        ...
        Empleado nuevoEmpleado = new Empleado(codigo, nombre, tarifa, sexo, horas);
        ...
        salida.mostrarMensaje(
                "Empleado agregado exitosamente\n\n" +
                        ...
                        "Salario Bruto: $" + String.format("%.2f", nuevoEmpleado.calcularSalarioBruto()) + "\n" +
